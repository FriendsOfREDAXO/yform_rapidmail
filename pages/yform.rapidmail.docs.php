...
$mdFiles = [];
foreach (glob(rex_addon::get('yform_rapidmail')->getPath('docs') . '/*.md') ?: [] as $file) {
...
$page = rex_be_controller::getPageObject('yform_rapidmail/docs');
...
if (null !== $page) {
    foreach ($mdFiles as $key => $mdFile) {
        ...
        $page->addSubpage(
            (new rex_be_page($key, rex_i18n::msg('yform_rapidmail_docs_' . $keyWithoudPrio)))
            ->setSubPath($mdFile)
            ->setHref('index.php?page=yform_rapidmail/docs&mdfile=' . $key)
            ->setIsActive($key == $currenMDFile),
        );
    }
}

echo rex_view::title($this->i18n('yform_rapidmail_title'));
